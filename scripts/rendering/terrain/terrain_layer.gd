...
var seed_value := -1
var grid := preload("res://scripts/rendering/terrain/terrain_grid.gd").new()
var clouds := preload("res://scripts/rendering/terrain/terrain_clouds.gd").new()
var hidden_areas := preload("res://scripts/rendering/terrain/hidden_areas.gd").new()
var cloud_edges := preload("res://scripts/rendering/terrain/terrain_cloud_edges.gd").new()
...
func synchronize(state: VigilState, camera: Vector2, zoom: float, viewport_size: Vector2) -> void:
	...
	var next_visible := {}
	for id in preload("res://scripts/rendering/terrain/region_query.gd").in_view(state.data.regions, view):
