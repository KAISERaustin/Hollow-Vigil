...
const GEAR := {
	...
	"cinder_censer": {"name": "Ashen Censer", "attack_count": 2.0, "dot_multiplier": 0.4, "duration": 3.0, "fire_damage": 1.0},
	"cinder_crucible": {"name": "Crucible Seal", "attack_count": 4.0, "blast_radius": 52.0},
	"bell_chain": {"name": "Undertow Chain", "attack_count": 4.0, "push_distance": 28.0, "push_immunity": 2.0},
	"bell_chime": {"name": "Hush Chime", "attack_count": 6.0, "duration": 0.6, "boss_duration": 0.2, "stun_immunity": 3.0},
	"prior_rosary": {"name": "Bloodmoon Rosary", "damage_per_stack": 6.0, "stack_limit": 6.0, "stack_timeout": 3.0},
	...
	"matriarch_lantern": {"name": "Wakekeeper’s Lantern", "opening_attacks": 2.0, "opening_speed": 35.0, "reset_timeout": 4.0}
}
...
const PRESENTATION := {
	...
	"prior": {"name": "Eclipse Shard", "boss": "prior", "symbol": "eclipse", "color": "d2adf3", "description": "Every {attack_count} attacks, the primary shot and its blast deal {damage_multiplier}× damage. Bypass boss defenses: {defense_bypass} (0 off, 1 on)."},
	"warden_thornspindle": {"name": "Briar Thornspindle", "boss": "warden", "symbol": "spindle", "color": "8aa970", "description": "Every {attack_count} attacks, wound the primary target for {dot_multiplier}× base damage per second for {duration}s. Fire damage: {fire_damage} (0 off, 1 on). Reapplying refreshes this tower’s wound."},
	...
	"cinder_censer": {"name": "Ashen Censer", "boss": "cindermaw", "symbol": "censer", "color": "d8754f", "description": "Every {attack_count} attacks, burn the primary target for {dot_multiplier}× base damage per second for {duration}s. Fire damage: {fire_damage} (0 off, 1 on). Reapplying refreshes this tower’s burn."},
	"cinder_crucible": {"name": "Crucible Seal", "boss": "cindermaw", "symbol": "seal", "color": "e6b16a", "description": "Every {attack_count} attacks, expand the primary shot’s blast to at least {blast_radius} units. All enemies in the blast take its normal damage."},
	...
	"bell_chime": {"name": "Hush Chime", "boss": "bell", "symbol": "chime", "color": "acd6cf", "description": "Every {attack_count} attacks, stun the primary target for {duration}s ({boss_duration}s for bosses). Immunity lasts {stun_immunity}s."},
	"prior_rosary": {"name": "Bloodmoon Rosary", "boss": "prior", "symbol": "rosary", "color": "cd8199", "description": "Repeated attacks on one target add {damage_per_stack}% primary-shot damage per stack, up to {stack_limit} stacks. Changing targets or pausing {stack_timeout}s resets them."},
	...
	"matriarch_lantern": {"name": "Wakekeeper’s Lantern", "boss": "mourning_matriarch", "symbol": "lantern", "color": "d7d1ae", "description": "On changing target or resuming after {reset_timeout}s, the first {opening_attacks} attacks gain {opening_speed}% attack speed."}
}
...
const ATTRIBUTES := {
	...
	"cinder_censer": ["damage_over_time"],
	"cinder_crucible": ["blast"],
	"bell_chain": ["knockback"],
	"bell_chime": ["stun"],
	"prior_rosary": ["momentum_damage"],
	"prior_mirror": ["fork"],
