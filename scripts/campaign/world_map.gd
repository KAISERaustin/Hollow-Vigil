...
func _ready() -> void:
	...
	for index in range(Catalog.COUNT):
		...
		button.landscape_profile = chapter_presentation(int(index/float(Catalog.LEVELS_PER_CHAPTER)))
		var landmarks: Array = button.landscape_profile.get("landmarks",[])
		if index % Catalog.LEVELS_PER_CHAPTER<4 and index % Catalog.LEVELS_PER_CHAPTER<landmarks.size(): button.landmark_kind = landmarks[index % Catalog.LEVELS_PER_CHAPTER]
		if index % Catalog.LEVELS_PER_CHAPTER == Catalog.LEVELS_PER_CHAPTER - 1:
			button.gate = Catalog.CHAPTERS[int(index / float(Catalog.LEVELS_PER_CHAPTER))].get("gate_art")
			button.gate_style = Catalog.CHAPTERS[int(index / float(Catalog.LEVELS_PER_CHAPTER))].style
...
func point(index: int) -> Vector2:
	var chapter := int(index / float(Catalog.LEVELS_PER_CHAPTER))
	var within := index % Catalog.LEVELS_PER_CHAPTER
	var x: float = [0.22, 0.74, 0.26, 0.70, 0.22, 0.74, 0.26, 0.77][within]
	# A reversed middle bend gives neighboring biomes their own trail rhythm.
	if chapter % 2 == 1 and within < Catalog.LEVELS_PER_CHAPTER - 1: x = 1.0 - x
...
func arrange() -> void:
	...
	for index in range(nodes.size()):
		var boss := index % Catalog.LEVELS_PER_CHAPTER == Catalog.LEVELS_PER_CHAPTER - 1
...
func chapter_roads(chapter: int) -> Array[PackedVector2Array]:
	...
	var first := chapter * Catalog.LEVELS_PER_CHAPTER
	...
	for index in range(first + 1, first + Catalog.LEVELS_PER_CHAPTER):
		roads.append(MapArt.between_markers(point(index - 1), point(index)))
	if chapter < Catalog.CHAPTERS.size() - 1:
		roads.append(MapArt.curve(point(first + Catalog.LEVELS_PER_CHAPTER - 1), Vector2(size.x - 16 * scale_x, top + CHAPTER_HEIGHT)))
	return roads
...
func _draw() -> void:
	for chapter in backgrounds.size():
		...
		var within := int(progress.data.completed_levels) - chapter * Catalog.LEVELS_PER_CHAPTER
		if within >= Catalog.LEVELS_PER_CHAPTER:
		...
		if progress.data.completed_levels < chapter * Catalog.LEVELS_PER_CHAPTER: continue
