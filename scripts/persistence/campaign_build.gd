...
static func valid(value: Variant) -> bool:
	...
	for tower in snapshot.towers.values():
		var allowed := int(tower.pad) >= 4 and preload("res://scripts/content/nodes/ground_placement.gd").allowed(snapshot, VigilWorld.pad_position(tower.region, tower.pad), tower.kind, run.mission.routes, run.game.economy.placement_bounds, tower.id)
