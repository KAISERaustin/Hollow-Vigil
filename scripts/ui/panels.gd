...
func clear_sheet(title: String, subtitle: String = "") -> void:
	...
	sheet_content.add_theme_constant_override("separation", 16 if mode == "reset" else 10)
	guide.hide()
	content_scroll.show()
	sheet_revision += 1
	prices.clear()
	...
	var heading := UI.heading(title, 23)
	heading.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	heading.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	row.add_child(heading)
	var close := UI.button("×", close_sheet, 44)
...
func fit_sheet() -> void:
	if not self.visible:
		return
	if mode == "reset":
		self.size.x = minf(440.0, app.size.x - 32.0)
		var content_height: float = sheet_content.get_parent().get_combined_minimum_size().y + get_theme_stylebox("panel").get_minimum_size().y
		self.size.y = minf(content_height, app.size.y - 32.0)
		self.position = (app.size - self.size) * 0.5
		return
	self.size.x = app.size.x - 24.0
	self.position.x = 12.0
	var heights := {"info": 580.0, "build": 380.0, "expand": 290.0, "rift": 470.0, "core": 310.0, "settings": 380.0}
	var desired_height: float = heights.get(mode, 340.0)
	if mode == "core":
		# Include the shared inner margins and panel padding, without unused space.
		desired_height = sheet_content.get_parent().get_combined_minimum_size().y + get_theme_stylebox("panel").get_minimum_size().y
...
func show_build() -> void:
	...
	for kind in Balance.TOWERS:
		...
		var b := UI.button(definition.name + "\n" + definition.role + "\n" + Balance.money(definition.cost) + " g", func(): selection_kind = kind; show_build(), 76)
		b.add_theme_font_size_override("font_size", 13)
		for state in ["normal", "hover", "pressed", "disabled"]:
			var style := UI.box(UI.SURFACE if state in ["normal", "disabled"] else UI.PANEL)
	...
	action_button = UI.gold_button("Build " + s.name + "  ·  " + Balance.money(s.cost) + " gold", func():
	...
	sheet_content.add_child(action_button)
	app.update_hud()
...
func show_expansion(id: String) -> void:
	...
	var revision := sheet_revision
	action_button = UI.gold_button("Claim territory  ·  " + Balance.money(action_cost) + " gold", func():
...
func show_entrance(id: String) -> void:
	...
	var revision := sheet_revision
	var traffic := UI.button("Increase traffic  ·  " + Balance.money(game.economy.traffic_cost(id)) + " g", func():
...
func show_settings() -> void:
	mode = "settings"
	clear_sheet("The quiet vigil", "Raise sentinels, gather gold, and expand your vigil beyond the mist.")
	sheet_content.add_child(UI.button("Power saving: " + ("30 FPS" if game.data.settings.low_power else "60 FPS"), func():
	...
	for stat in [["Lifetime gold", game.data.lifetime_earnings], ["Escaped", game.data.escapes]]:
		...
		field.add_theme_stylebox_override("panel", UI.box(UI.SURFACE))
...
func show_reset_confirmation() -> void:
	...
	sheet_content.add_child(UI.accent_button("Reset and start over", app.reset_progress, UI.DANGER))
	sheet_content.add_child(UI.button("Cancel", show_settings))
