...
static func draw(art: Control, kind: String) -> void:
	if kind == "gold":
		...
		for height in [7, 0, -7]:
			VigilTerrainArt.ellipse(art, center + Vector2(-10, height) * scale, Vector2(13, 6) * scale, VigilTerrainArt.GOLD, 2 * scale)
		var coin := center + Vector2(11, 0) * scale
		VigilTerrainArt.disk(art, coin, 13 * scale, VigilTerrainArt.GOLD, 2 * scale)
		art.draw_arc(coin, 9 * scale, 0, TAU, 32, VigilTerrainArt.INK, scale, true)
		VigilTerrainArt.shape(art, [Vector2(0, -5), Vector2(3, 0), Vector2(0, 5), Vector2(-3, 0)], coin, Vector2.ONE * scale, VigilTerrainArt.INK, 0)
		return
