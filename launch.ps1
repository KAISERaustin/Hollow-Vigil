...
try {
    ...
    if ($Editor) {
        ...
        if ($MobileTests -or $Check) {
            ...
            Invoke-Godot -Name 'manual-cloud' -EngineArguments @('--script', 'res://tests/rendered/manual_cloud_runner.gd')
        ...
        if ($ArtSmoke -or $Check) {
			Invoke-Godot -Name 'ironspike-art' -EngineArguments @('--script', 'res://tests/rendered/ironspike_art_runner.gd')
