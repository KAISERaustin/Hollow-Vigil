...
Building opens from a centered upward caret over a thin bottom parchment strip, with a 48-unit touch target. The caret opens a full-width, bottom-aligned drawer with a 180 ms upward slide. Its only contents are horizontally scrollable tower cards and exact prices; omit the heading, instruction sentence, and Close button. Outside taps or system Back dismiss the drawer. Keep its cards inside safe-area padding while the enclosure reaches both screen edges. Dragging or selecting a card retains the compact placement preview with native tower artwork and a four-square horizontal level indicator. The four vector image states fill one through four squares for the owned tower level; all four indicate maximum level. Construction shows level 1. Specialization choices appear only when an existing tower reaches level 3. Full inspection and upgrade comparisons retain their statistics.
