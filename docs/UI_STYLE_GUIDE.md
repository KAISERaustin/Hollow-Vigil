...
Building opens from a raised square tab at the bottom left containing an upward caret, seated above a 24-unit bottom parchment strip, with a 48-unit touch target. The caret opens a full-width, bottom-aligned drawer with a 180 ms upward slide. Its only contents are horizontally scrollable tower cards and exact prices; omit the heading, instruction sentence, and Close button. Outside taps or system Back slide the drawer down over 180 ms before hiding it; dismissal input does not reach the battlefield. The horizontal card viewport reaches both screen edges, with no inset clipping gutter. Put 8-unit leading and trailing padding inside the scrolling contents, matching the card gap, so it scrolls away. Use 24-unit top and bottom drawer padding. Retain bottom safe-area clearance. Dragging or selecting a card retains the compact placement preview with native tower artwork and a four-square horizontal level indicator. The four vector image states fill one through four squares for the owned tower level; all four indicate maximum level. Construction shows level 1. Specialization choices appear only when an existing tower reaches level 3. Full inspection and upgrade comparisons retain their statistics.
