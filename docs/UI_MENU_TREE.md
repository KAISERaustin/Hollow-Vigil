...
├── Contents → applicable checkboxes, selected by default
│   ├── Game rules and resources → all enemies, bosses, towers, gear, rifts, starting resources, and Campaign wave settings
│   └── Layout and equipment [Infinite only] → placed towers, upgrades, equipment, and all explored tiles
├── Name / Description
├── Save privately
...
Main menu → Settings → Change log appears directly below Bug report. It groups changes beneath readable date headings with larger bulleted text, plus Refresh and Load older changes. Back returns to Settings; no sign-in is required. See CHANGE_LOG.md for backend editing and the September 8 import.
