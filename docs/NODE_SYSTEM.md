...
| Boss â†’ Enemy | Encounter state; overridable movement speed and damage absorption | Encounter service owns discovery, routing, regeneration and escort timers |
...
The `build_contents` family in `content/catalogs/build_groups.gd` registers common content categories. Each `BuildGroupNode` declares its game-type applicability, export availability, label, tuning category, and selection group. The Save build checklist offers all game rules and starting resources (including Campaign wave settings), plus tower layout with equipment and explored tiles for Infinite only. Campaign exports exclude placed towers and owned equipment at capture time. Export availability is separate from validation so older imported builds and complete saved games retain their layouts. `ui/shared/contents_checklist.gd` owns only one form's selection state and omits unavailable groups. The rules option discovers every registered type, including tower tiers, without new menu branches. The serialized categories remain independent so older partial builds still load; reopening one in Save build expands any selected category to its complete checkbox group. `tests/unified_persistence_runner.gd` verifies registered extensions, grouped saves, older partial contents, and independent session state.
...
Ruined King and Mourning Matriarch inherit the shared Boss â†’ Enemy node, with distinct stats, knockback resistance and native silhouettes. Their rewards are owned by the shared progression and equipment catalogs. Their sound families are assigned through boss presentation rules. No new special ability or mutable shared state is introduced. Optional future abilities should use attachable components.
