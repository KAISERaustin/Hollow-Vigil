...
Attack reach belongs to each Tower content type in `content/catalogs/towers.gd`. Pyre uses 115/129/143 units at levels 1/2/3 to trade reach for splash damage; Ashneedle uses 140/154/168 for rapid single-target darts; Stormspire uses 160/174/188 for multi-target coverage; Obelisk uses 185/201/217 for slow, heavy long-range shots. Level 4 inherits its family's level-3 reach unless explicitly tuned. Combat, placed range outlines, and build previews resolve the same content stats and session tuning. Opening either build menu previews its selected card at the empty socket, including the configured range outline. Shared build cards display each type's configured base range.

Campaign and Infinite build menus use `ui/towers/tower_choice.gd`'s shared `build_list` factory. Its horizontal strip contains 112-pixel-tall profile cards with a bold title, separate gold/range badges and a selected checkmark. The reusable `content_portrait.gd` profile factory frames the battlefield artwork in a medallion tinted from the tower's catalog color. Touch swipes, keyboard navigation and focus scrolling expose the complete strip; both menus fit the card height and keep the Build button pinned below. The factory enumerates `Balance.TOWERS`, so adding a registered base Tower type automatically adds a fully styled portrait card in both modes without changing either host or increasing menu height. Hosts own selection and construction callbacks; the shared component owns card layout and selection highlighting through `select`. Both modes select a preview first and construct only through the pinned Build button; Campaign still applies mission affordability through its run owner.
...
The battlefield also owns an independent `rendering/build_preview.gd` presentation component. It draws the selected base tower with the existing sentinel renderer and frames the tower and full range above the build menu using the same camera transition. The list height stays stable across choices and scrolls on short screens while the confirmation stays pinned. During preview, a wider overview is allowed within camera bounds; Campaign temporarily includes one extra tile of surrounding terrain for framing. Closing, construction or changing panels clears the preview, and manual camera input cancels the drift. Preview selection never creates gameplay records or spends gold.
