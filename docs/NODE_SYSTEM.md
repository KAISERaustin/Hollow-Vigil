...
The `build_contents` family in `content/catalogs/build_groups.gd` registers common content categories. Each `BuildGroupNode` declares its game-type applicability, label, tuning category, and selection group. The Save build checklist exposes two options, both selected by default: all game rules and starting resources (including Campaign wave settings), and tower layout with equipment and explored Infinite tiles. `ui/shared/contents_checklist.gd` owns only one form's selection state. The rules option discovers every registered type, including tower tiers, without new menu branches. The serialized categories remain independent so older partial builds still load; reopening one in Save build expands any selected category to its complete checkbox group. `tests/unified_persistence_runner.gd` verifies registered extensions, grouped saves, older partial contents, and independent session state.
