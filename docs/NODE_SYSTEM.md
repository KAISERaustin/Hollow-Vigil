...
Campaign and Infinite build menus use `ui/towers/tower_choice.gd`'s shared `build_list` factory. Each card shows the base tower through the battlefield's sentinel renderer, its configured title and exact gold cost. Hosts own selection and construction callbacks; the shared catalog owns card layout and identity. Both modes select a preview first and construct only through the pinned Build button; Campaign still applies mission affordability through its run owner.
