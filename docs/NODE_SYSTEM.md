...
Campaign run owns an attempt-local auto-start playback preference, off by default.
The shared battlefield skip toggle enables subsequent waves; the first wave keeps
its manual Start wave action. Presentation waits for rewards, pause, tutorials and
active menus or construction before using the normal wave-start path. Every new
attempt starts with auto-start off; this playback preference is not saved.
