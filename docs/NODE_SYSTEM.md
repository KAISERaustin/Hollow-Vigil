...
Campaign and Infinite build menus use `ui/towers/tower_choice.gd`'s shared `build_list` factory. Its horizontal strip contains 112-pixel-tall cards with the battlefield's sentinel artwork, configured title, exact gold cost and range. Touch swipes, keyboard navigation and focus scrolling expose the complete strip; both menus fit the card height and keep the Build button pinned below. The factory enumerates `Balance.TOWERS`, so adding a registered base Tower type automatically adds a card in both modes without changing either host or increasing menu height. Hosts own selection and construction callbacks; the shared component owns card layout and selection highlighting through `select`. Both modes select a preview first and construct only through the pinned Build button; Campaign still applies mission affordability through its run owner.
