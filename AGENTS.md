...
- Button borders use `VigilInterface.BUTTON_OUTLINE` = **2 UI units**; all other visible enclosures and dividers use `VigilInterface.OUTLINE` = **3 UI units**, color `#000000`. Use `UI.button_surface()` for buttons so the saved Settings → Button colors palette applies. Panels, cards, stat cells, fields, badges and dividers retain their shared owner. Neither border may thicken on interaction. Keep rectangular corners at 4 units (0 for edge-to-edge chrome). World artwork contours remain governed by `docs/ART_DIRECTION.md`.
