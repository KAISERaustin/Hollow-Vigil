...
- Before adding or changing UI, read [docs/UI_THEME.md](docs/UI_THEME.md), then the relevant recipe in [docs/UI_STYLE_GUIDE.md](docs/UI_STYLE_GUIDE.md). These define one shared Pickard theme: moonlit charcoal-green illustration, parchment lettering and reading surfaces, restrained diamond identity marks, gold-paper actions, and compact ink-bordered cards. The opening screen is part of this style, not a separate exception. The [Waves reference](docs/references/ui-waves-reference.png) defines card information hierarchy; the Pickard reference defines the shared illustrated identity.
- For repeated option rows, keep descriptions and portraits on the left, explicit action controls grouped on the right, and one 3-unit black divider below every row. Reuse `UI.action_row()` and `UI.number_row()`; preserve passive swipe space, aligned action columns and 48-unit targets. Follow `UI_THEME.md` for multiple controls, wrapping, fields, fixed navigation and scrolling.
