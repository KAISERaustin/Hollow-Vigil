...
class Bodies extends Node2D:
	...
	var actors := []
	var groups := []
	func configure(count: int, kinds: Array) -> void:
		var by_region := {}
		...
		for members in by_region.values():
			...
			for index in range(members.size()):
				instances.set_instance_transform_2d(index, Transform2D(0.0, members[index].at + Vector2(0,-0.5)))
...
func run() -> void:
	...
	file.store_string(JSON.stringify({"diagnostic_only": true, "overlapping_indicators_supported": false, "rows": rows}, "\t"))
	quit()
