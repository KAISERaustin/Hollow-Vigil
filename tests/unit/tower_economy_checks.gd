...
static func run(suite: SceneTree) -> void:
	...
	suite.check(Balance.sell_refund(g.data.towers[id]) == 110.0, "Level-three refund is half the build price plus both upgrade costs")
	...
	suite.check(g.economy.sell(id, 2).is_empty() and g.data == before, "Stale sale level cannot remove or pay for a tower")
	suite.check(g.economy.sell("missing").is_empty() and g.data == before, "Missing tower cannot be sold")
	var result := g.economy.sell(id, 3)
	suite.check(result == {"refund": 110.0, "earnings": 17.5, "total": 127.5}, "Sale includes every stored gold fraction")
	suite.check(g.data.balance == before.balance + 127.5 and not g.data.towers.has(id), "Sale atomically removes the tower and pays once")
	...
	suite.check(g.economy.sell(id, 3).is_empty() and not g.economy.upgrade(id, 3) and g.economy.collect(id) == 0.0 and g.data == sold, "Repeated sale and stale tower actions cannot pay again")
