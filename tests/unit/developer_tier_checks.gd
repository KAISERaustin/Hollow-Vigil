...
static func test_abilities(suite: SceneTree) -> void:
	...
	suite.check(game.combat.pending_shots.size() == 1, "Legacy fan configuration is accepted without restoring retired spread")
	game.set_tower_tier_stat("rapid:thorn_volley", "duration", 4.0)
	game.set_tower_tier_stat("rapid:thorn_volley", "dot_multiplier", 2.0)
	game.combat.pending_shots.clear()
	game.combat.launch_shot(game.data.towers["1"], Vector2.ZERO, e, Balance.tower_stats(game.data.towers["1"], game.tuning))
	game.combat.advance_shots(1.0)
	suite.check(e.gear_status.values()[0].until == 4.0 and e.gear_status.values()[0].damage == 30.0, "Poison duration and damage settings reach the shared status system")
