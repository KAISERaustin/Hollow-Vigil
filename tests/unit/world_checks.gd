...
static func test_world(suite: SceneTree) -> void:
	...
	for i in range(80):
		...
		for kind in Balance.portal_kinds(r.style):
...
static func test_territory_styles(suite: SceneTree) -> void:
	...
	for i in range(1, 61):
		...
		suite.check(style in VigilWorld.ALL_STYLES, "New territory selects a supported theme")
		seen[style] = true
	suite.check(seen.has("forest") and seen.has("castle_ruin") and seen.has("ashen_forge") and seen.has("drowned_crypt") and seen.has("bloodmoon_sanctuary"), "Purchased territories include all four biomes and castle ruins")
