...
func fixture(kind: String, branch: String) -> VigilState:
	...
	g.expand("-1,0")
	# Branch fixtures measure authored targets without the tile's new encounter.
	g.combat.enemies.clear()
	g.data.regions["-1,0"].erase("boss")
...
func run() -> void:
	...
	g.combat.launch_shot(g.data.towers["1"],origin,e,Balance.tower_stats(g.data.towers["1"]))
	check(g.combat.pending_shots.size() == 1,"Poison Arrow launches one aimed projectile")
	other = enemy(g,origin+Vector2(0,-25)+Vector2.from_angle(0.24)*70)
	g.combat.advance_shots(1)
	check(e.hp == 9985 and other.hp == 10000,"Poison Arrow hits selected target without stray spread")
	check(e.gear_status.size() == 1 and e.gear_status.values()[0].damage == 5.0, "Poison Arrow attaches shared damage over time")
