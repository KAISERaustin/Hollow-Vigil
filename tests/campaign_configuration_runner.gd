...
func run() -> void:
	...
	check(Configuration.valid_level(9, custom), "Per-level and per-wave overrides validate")
	var store := Configuration.new()
	store.path = "user://campaign-config-test-" + str(Time.get_ticks_usec()) + ".save"
	check(store.save_level(9, custom), "Campaign configuration persists separately")
	var reloaded := Configuration.new()
	reloaded.path = store.path
	reloaded.load_configuration()
	check(not reloaded.blocked and reloaded.overrides(9) == JSON.parse_string(JSON.stringify(custom)), "Per-level configuration survives reload")
	var battle := Run.new(9, reloaded.overrides(9))
	...
	for bad in [{"gold": -1}, {"flame": 0}, {"reward": INF}, {"tuning": {"session": {"start": {"starting_gold": 1}}}}, {"waves": {"99": {}}}, {"wave_count": 1, "waves": {"0": {"groups": []}}}, {"waves": {"0": {"groups": [["bad", 1, 0, 0, 1]]}}}, {"waves": {"0": {"groups": [["basic", 1, 9, 0, 1]]}}}, {"towers": {}}]:
		check(not Configuration.valid_level(9, bad), "Malformed campaign configuration rejected")
		check(Run.new(9, bad).game.data.balance == Configuration.Catalog.level(9).gold, "Malformed overrides safely retain defaults")
	check(store.save_level(9, {}) and store.overrides(9).is_empty(), "Reset configuration restores authored values only")
	check(store.data.keys().size() == 4 and not store.data.has("completed_levels"), "Configuration schema excludes progression")
