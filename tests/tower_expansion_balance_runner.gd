...
func sample(kind: String, stage: Array, paired: bool = false, opening: bool = false) -> Dictionary:
	...
	for seed_value in [123, 314, 879]:
		for direction in VigilWorld.DIRS:
			...
			game.data.regions[region].traffic = 0 if opening else Balance.MAX_TRAFFIC_LEVEL
			game.data.regions[region].unlocks = [] if opening else ["fast", "heavy"]
...
func run() -> void:
	...
	for kind in Kinds:
		if kind != "hex_lantern":
			var opening := sample(kind, [1, ""], false, true)
			report += row("basic-opening", kind, [1, ""], opening)
			print("OPENING SAMPLE ", kind, " clear=", opening.clear)
			check(opening.clear >= 0.5, kind + " can earn from a basic opening without support")
		var previous := 0.0
		for stage in Integration.stages(kind):
			...
			# Weak initial marks need a durable target or several allies to cross
			# kill thresholds; exact allied amplification is checked in combat.
			if kind == "hex_lantern" and stage[0] >= 3: check(result.gold > ally.gold, label + " improves the same allied defense")
