...
func run() -> void:
	...
	for viewport in [Vector2i(360, 640), Vector2i(390, 844), Vector2i(540, 960)]:
		...
		await settle()
		check(app.field.preview_kind == app.panels.build_selection.kind and app.field.selected_range() > 0.0, "Opening the icon picker previews the remembered tower")
		...
		for choice in choices.get_children():
			...
			for repeat in range(2):
				...
				check(root.gui_get_focus_owner() == app.panels.build_back, "details focus their Back button")
				...
				check(is_equal_approx(app.field.selected_range(), definition.range), "Preview must use selected tower range")
