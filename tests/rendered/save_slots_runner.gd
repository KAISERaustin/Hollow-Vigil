...
func run() -> void:
	...
	check(upload.get_parent() == app.panels.sheet_content.get_child(app.panels.sheet_content.get_child_count() - 1), "Upload build is at bottom of Creative settings")
	upload.pressed.emit()
	await frame()
	...
	app.slot_menu.find_child("SaveConfiguration", true, false).pressed.emit()
	...
	var modes: OptionButton = app.slot_menu.find_child("CreationMode", true, false)
	modes.select(1)
