...
const STAGES = [[1, "", "tier-1"], [2, "", "tier-2"], [3, "", "tier-3"],
	[4, "tempest_web", "tier-4-skyfork"], [4, "thunderseal", "tier-4-thunderward"]]
...
class SpriteCheck extends Node2D:
	...
	func _draw() -> void:
		...
		if mode == "direct":
			...
			var entry: Dictionary = images.entries["tower/electric/%d/%s" % [stage[0], stage[1]]]
			...
			VigilTerrainArt.sentinel_vector(self, "electric", at, zoom, stage[0], stage[1])
		else:
			VigilTerrainArt.sentinel(self, "electric", at, zoom, stage[0], stage[1])
...
func run() -> void:
	...
	for stage in STAGES:
		var key := "tower/electric/%d/%s" % [stage[0], stage[1]]
		var entry: Dictionary = images.entries[key]
		var source := "res://docs/concepts/stormspire-tempest-new/%s.png" % stage[2]
		...
		check((origin + Vector2(627, 1164) / entry.pixels_per_unit).is_zero_approx(), key + " shared ground anchor")
		var picture := texture.get_image()
		var muzzle: Vector2 = (Balance.PROJECTILES.electric.muzzle - origin) * entry.pixels_per_unit
		var inset := picture.get_pixelv(Vector2i(muzzle))
		check(inset.a > 0.9 and inset.b > inset.r and inset.g > inset.r, key + " lightning starts at enchanted staff inset")
	...
	for stage in STAGES:
		for dimension in [48, 80]:
			...
			var portrait := Portrait.preview("towers", "electric", stage[0], stage[1])
	...
	for dimensions in [Vector2i(360, 640), Vector2i(390, 844), Vector2i(540, 960)]:
		...
		for index in range(STAGES.size()):
			...
			var id := str(index)
			board.state.data.towers[id] = {"id": id, "kind": "electric", "level": stage[0],
			...
			var label := Label.new()
			label.text = "Stormspire %d" % stage[0] if stage[1] == "" else Balance.BRANCHES.electric[stage[1]].name
			label.position = Vector2(dimensions.x * 0.52, 130 + index * 105)
			label.add_theme_font_size_override("font_size", 13)
			board.add_child(label)
		board.queue_redraw()
		await frame()
		check(root.get_texture().get_image().save_png("res://artifacts/stormspire-%dx%d.png" % [dimensions.x, dimensions.y]) == OK, "Saved battlefield and portrait capture")
		board.free()
	print("STORMSPIRE: %d checks, %d failures" % [checks, failures.size()])
