...
func run() -> void:
	...
	for viewport in [Vector2i(360,640), Vector2i(390,844), Vector2i(540,960), Vector2i(844,390)]:
		...
		for mode in ["creative", "survival"]:
			...
			check(whole.data.levels["0"].layout.towers.size() == 1, "Map export includes checkpoint towers without a live run")
			...
			whole.data.levels["0"].layout.towers.clear()
			...
			check(menu.prepared_form().data.levels["0"].layout.towers.size() == 3, "Returning from battle exports the latest live layout")
