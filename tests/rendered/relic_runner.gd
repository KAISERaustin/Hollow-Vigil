...
func run() -> void:
	...
	for touch in [false, true]:
		...
		check(dialog.header_back.size == Vector2(48, 48) and dialog.header_back.global_position - dialog.card.global_position == Vector2(12, 12), "Equipment details use the shared upper-left Back geometry")
		await Harness.tap(app, dialog.header_back.get_global_rect().get_center(), touch)
		check(dialog.mode == "equipment" and app.game.data == before, "Equipment Back returns without applying the selection")
