...
class SpriteCheck extends Node2D:
	...
	func _draw() -> void:
		...
		if mode == "direct":
			...
			var entry: Dictionary = images.entries["tower/hex_lantern/%d/%s" % [stage[0], stage[1]]]
			...
			VigilTerrainArt.sentinel_vector(self, "hex_lantern", at, zoom, stage[0], stage[1])
		else:
			VigilTerrainArt.sentinel(self, "hex_lantern", at, zoom, stage[0], stage[1])
...
func run() -> void:
	...
	for stage in STAGES:
		var key := "tower/hex_lantern/%d/%s" % [stage[0], stage[1]]
		var entry: Dictionary = images.entries[key]
		var source := "res://docs/concepts/hex-lantern-witchlight-2026-09-20/%s.png" % stage[2]
	...
	for dimensions in [Vector2i(360, 640), Vector2i(390, 844), Vector2i(540, 960)]:
		...
		for index in range(STAGES.size()):
			...
			var id := str(index)
			board.state.data.towers[id] = {"id": id, "kind": "hex_lantern", "level": stage[0],
			...
			var label := Label.new()
			label.text = "Hex Lantern %d" % stage[0] if stage[1] == "" else Balance.BRANCHES.hex_lantern[stage[1]].name
			label.position = Vector2(dimensions.x * 0.52, 130 + index * 105)
			label.add_theme_font_size_override("font_size", 13)
			board.add_child(label)
		board.queue_redraw()
		await frame()
		check(root.get_texture().get_image().save_png("res://artifacts/hex-lantern-%dx%d.png" % [dimensions.x, dimensions.y]) == OK, "Saved battlefield and portrait capture")
		board.free()
	print("HEX LANTERN ART: %d checks, %d failures" % [checks, failures.size()])
