...
func run() -> void:
	...
	for dimensions in [Vector2i(360, 640), Vector2i(390, 844), Vector2i(540, 960)]:
		...
		for type in ["campaign", "infinite"]:
			...
			check(menu.content.find_children("Contents_*", "CheckBox", true, false).size() == 2, "Save build has exactly two content options")
			check(button("Contents_rules").button_pressed and button("Contents_layout").button_pressed, "Both content options start selected")
			...
			check(menu.form.contents == ({"layout": true, "terrain": true} if type == "infinite" else {"layout": true}), "Layout, equipment and explored tiles toggle together without rules")
			check(not menu.prepared_form().is_empty(), "Layout option alone produces a valid build")
