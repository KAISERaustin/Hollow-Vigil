...
func run() -> void:
	...
	for dimensions in [Vector2i(360,640), Vector2i(390,844), Vector2i(540,960)]:
		...
		check(app.slot_menu.form.levels.size() == preload("res://scripts/campaign/catalog.gd").COUNT, "Share includes every campaign level")
		var title: LineEdit = app.slot_menu.find_child("BuildName", true, false)
		title.text = "Phone campaign " + str(dimensions.x)
		title.text_changed.emit(title.text)
		app.slot_menu.find_child("SavePrivately", true, false).pressed.emit()
		check(not app.slot_menu.form_saved_code.is_empty(), "Whole campaign saved with shared export UI")
		app.slot_menu.form.return.call()
		screen.show_setup()
		...
		screen.show_playthrough_share()
		check(app.slot_menu.form.levels["0"].overrides.gold == 5432.0, "Export uses newly saved setup rules instead of a stale mission")
		check(app.slot_menu.slots.save_shared(Playthrough.encode(screen.session_levels(), "Legacy campaign " + str(dimensions.x), "", true)), "Keep legacy setup-library compatibility covered alongside current exports")
		screen.go_back()
