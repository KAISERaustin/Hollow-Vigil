...
const STAGES = [[1, "", "tier-1"], [2, "", "tier-2"], [3, "", "tier-3"],
	[4, "cinderfield", "tier-4-ember-censers"], [4, "rupture_pyre", "tier-4-flame-seal"]]
...
class SpriteCheck extends Node2D:
	...
	func _draw() -> void:
		...
		if mode == "direct":
			...
			var entry: Dictionary = images.entries["tower/splash/%d/%s" % [stage[0], stage[1]]]
			...
			VigilTerrainArt.sentinel_vector(self, "splash", at, zoom, stage[0], stage[1])
		else:
			VigilTerrainArt.sentinel(self, "splash", at, zoom, stage[0], stage[1])
...
func run() -> void:
	...
	for stage in STAGES:
		var key := "tower/splash/%d/%s" % [stage[0], stage[1]]
		var entry: Dictionary = images.entries[key]
		var source := "res://docs/concepts/pyre-ember-shrine/%s.png" % stage[2]
		...
		check((origin + Vector2(627, 1130) / entry.pixels_per_unit).is_zero_approx(), key + " shared ground anchor")
		var picture := texture.get_image()
		var muzzle: Vector2 = (Balance.PROJECTILES.splash.muzzle - origin) * entry.pixels_per_unit
		var flame := picture.get_pixelv(Vector2i(muzzle))
		check(flame.a > 0.9 and flame.r > 0.6 and flame.r > flame.g and flame.g > flame.b, key + " muzzle inside open flame")
	...
	for stage in STAGES:
		for dimension in [48, 80]:
			...
			var portrait := Portrait.preview("towers", "splash", stage[0], stage[1])
	...
	for dimensions in [Vector2i(360, 640), Vector2i(390, 844), Vector2i(540, 960)]:
		...
		for index in range(STAGES.size()):
			...
			var id := str(index)
			board.state.data.towers[id] = {"id": id, "kind": "splash", "level": stage[0],
			...
			var label := Label.new()
			label.text = "Pyre %d" % stage[0] if stage[1] == "" else Balance.BRANCHES.splash[stage[1]].name
			label.position = Vector2(dimensions.x * 0.52, 130 + index * 105)
			label.add_theme_font_size_override("font_size", 13)
			board.add_child(label)
		board.queue_redraw()
		await frame()
		check(root.get_texture().get_image().save_png("res://artifacts/pyre-%dx%d.png" % [dimensions.x, dimensions.y]) == OK, "Saved battlefield and portrait capture")
		board.free()
	print("PYRE: %d checks, %d failures" % [checks, failures.size()])
