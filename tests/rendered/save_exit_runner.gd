...
func check_campaign_exit() -> void:
	...
	app.show_game_menu()
	await press("ExitGame")
	check(menu.screen == "home" and not is_instance_valid(app.campaign) and button("ConfirmAction") == null, "Successful Campaign save exits directly")

func check_exit_popup_fit() -> void:
	var action := button("ConfirmAction")
	if action == null: return
	var popup := action.get_window()
	var viewport := Rect2(Vector2.ZERO, Vector2(root.size))
	check(viewport.encloses(Rect2(Vector2(popup.position), Vector2(popup.size))), "Exit warning fits mobile viewport")
	check(Rect2(Vector2.ZERO, Vector2(popup.size)).encloses(action.get_global_rect()), "Exit anyway remains visible in warning")
