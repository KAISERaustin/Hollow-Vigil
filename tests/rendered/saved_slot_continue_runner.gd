...
func check_campaign_continue() -> void:
	...
	for mode in ["survival", "creative"]:
		...
		for dimensions in [Vector2i(360, 640), Vector2i(390, 844), Vector2i(540, 960)]:
			...
			check(saved_games != null and saved_games.text == "Saved Games", "Campaign map labels its saved games destination")
			...
			check(app.campaign.page == "map" and not menu.visible, "Level Menu returns directly to the map")
			...
			check(is_equal_approx(toolbar.menu_button.global_position.x, toolbar.global_position.x), "Menu sits at the far left")
			check(toolbar.pause_button.global_position.x > toolbar.menu_button.get_global_rect().end.x, "Playback sits to the right of Menu")
			...
			await press("GameMenuButton")
			check(app.campaign.page == "map" and not menu.visible, "Running level Menu also returns directly to map")
			await press("CampaignSavedGames")
