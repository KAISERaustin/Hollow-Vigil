...
func run() -> void:
	...
	check(Configuration.valid_level(9, rules), "Populated level can retain empty placeholders")
	var resized := Editor.set_count(9, rules, 1, "basic", 15)
	...
	check(not Configuration.valid_level(9, Editor.set_count(9, rules, 1, "basic", 0)), "Cannot remove last enemy from level")
	check(not Configuration.valid_level(9, Editor.remove_wave(9, rules, 1)), "Cannot remove last populated wave")
	for mode in ["creative", "survival"]:
		var battle := Run.new(9, rules, mode)
	...
	var battle := Run.new(9, {}, "creative")
	battle.wave = 2
	check(battle.apply_configuration(Editor.remove_wave(9, {}, 0), 0) and battle.wave == 1, "Deleting cleared wave remaps live progress without replay")
	var many := {}
	...
	check(store.save_level(9, resized), "Custom composition persists")
	var reload := Configuration.new()
	reload.path = store.path
	reload.load_configuration()
	check(reload.overrides(9) == JSON.parse_string(JSON.stringify(resized)), "Custom waves survive reload")
	var build := Build.capture("campaign", battle.game, {"9": {"overrides": resized}}, "level", 9, {"timing": true, "composition": true, "rewards": true}, "Wave editor", "")
	...
	if not build.is_empty():
		var restored := Build.campaign_level(build, 9)
		check(restored.ok and Configuration.resolve(9, restored.level.overrides).waves == Configuration.resolve(9, resized).waves, "Imported build reproduces exact composition")
	var frozen := Playthrough.freeze_level(9, resized)
	check(Configuration.valid_level(9, frozen) and frozen.wave_count == 3 and frozen.waves["0"].groups.is_empty(), "Full campaign freeze preserves custom shape")
