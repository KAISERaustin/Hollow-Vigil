...
- Every purchased portal continuously spawns enemies. Orchard and castle patches each have one portal; their other tiles provide roads and tower sockets. All enemies follow connected roads to the core; escapes cause no damage or gold loss.
...
Only one tile in the patch has a portal. It summons **Briarlings** (85 HP, 86 speed, 13 gold), **Veil Widows** (260 HP, 43 speed, 28 gold), and **Coffinbound** (820 HP, 22 speed, 64 gold), with equal chances and no attunement cost. Their knockback resistance is 0%, 25%, and 90%, respectively. They never appear in ordinary rifts, dungeon portals or boss escorts. Portal traffic, towers, targeting, specializations, relics, bounty collection, offline income and camera-independent simulation use the existing systems.
...
In Creative, Settings → Upload build lets you name a configuration, describe your changes, and save it directly to a local library. It includes the world, towers, resources and tuned values. To reuse one, open an empty save, choose a saved configuration from the scrollable library, select **Creative or Survival**, and press **Create** at the bottom. You can also start with a fresh world. Creative worlds remain editable; Survival worlds keep the chosen rules locked. Configurations receive no offline earnings for time since capture. Cloud identities and personal preferences are excluded. Saving the same name again keeps a separate copy. Resetting Survival progress retains its custom rules. Saves and configurations are local until you choose **Sync this save** under Account & cloud saves. Cloud backups preserve Creative/Survival mode, configuration details, and custom balance rules. Sound controls are under Settings → Sound.

Public Builds: **Upload build**, at the bottom of Settings, saves a Creative configuration locally and queues it for public upload. Sign in and set your player name under Account & cloud saves to publish; offline uploads retry every minute. Settings → Public Builds and the save creation menu show shared titles, descriptions, author names at publication, and export timestamps in UTC. Choose a build, an empty save slot, and Creative or Survival. Existing saves are preserved. Builds include world layout, resources, towers, equipment and tuning, but omit cloud identity and personal settings.

## Cloud acceptance testing

Run `python3 tools/cloud_qa.py` for a separate QA save library and a **Sign in to QA account** button. It uses normal Supabase password authentication for a dedicated, pre-verified test account. Credentials are stored outside the repository at `~/.config/hollow-vigil/qa-account.json` with owner-only permissions. No QA login or credentials are included in exported games. To configure another machine, create and auto-confirm a QA user in Supabase Auth, then store its email, password and project reference in that local file; `--create-account` can provision it when the signed-in Supabase CLI has the required admin permissions.
